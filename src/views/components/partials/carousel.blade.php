...
<div x-data="{items: [{{$value}}], active: 0}">
    <div class="snap overflow-auto relative flex-no-wrap flex transition-all" x-ref="slider"
        ...
        <template x-for="(item,index) in items" :key="index">
            <div class="w-full flex-shrink-0 h-32 bg-black text-white flex items-center justify-center">
                <img @click="$dispatch('img-modal', {  imgModalSrc: item })" :src="item" src="" />
            </div>
        </template>
    </div>
    <div class="p-4 flex items-center justify-center flex-1 bg-blue-500 bg-opacity-75">
