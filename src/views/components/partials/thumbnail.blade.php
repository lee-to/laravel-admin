...
@elseif($type == "multiple")
    <div @if($attr["removeable"]) x-data="{}" x-ref="hidden_{{ $attr["name"] }}_parent" @endif class="relative bg-white p-3 m-2 border-1 border-dashed border-gray-100 shadow-md rounded-lg overflow-hidden">
        ...
        <img @click="$dispatch('img-modal', {  imgModalSrc: '{{ Storage::url($value) }}' })" class="w-full object-cover object-center rounded" src="{{ Storage::url($value) }}" />
...
@else
    <div x-data={} class="max-w-sm rounded overflow-hidden shadow-lg mt-5">
        <img @click="$dispatch('img-modal', {  imgModalSrc: '{{ Storage::url($value) }}' })" class="w-full" src="{{ Storage::url($value) }}" />
    </div>
@endif
